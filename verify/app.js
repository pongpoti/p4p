...
(function () {
    ...
    async function establishSession(accessToken, refreshToken) {
        ...
        if (!resp.ok)
            throw new Error("session POST failed: " + resp.status);
    }
    ...
    const readPending = () => {
        try {
            ...
            if (p && p.email && Date.now() - p.ts < PENDING_TTL)
                return p.email;
        }
        catch (e) { /* ignore */ }
        clearPending();
    ...
    emailStep.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const { data: allowed, error: rpcErr } = await db.rpc("is_sender_allowlisted", { p_email: email });
            if (rpcErr)
                throw rpcErr;
            if (!allowed) {
