/** GENERATED FILE'S SOURCE — this file is compiled to verify/app.js by
...
;(function () {
    ...
    const liffReady: Promise<boolean> = liff.init({ liffId: LIFF_ID }).then(() => true).catch((err) => {
    ...
    // Shape of the JSON supabase/functions/line-verify answers with — loose
    // on purpose (only the fields this page actually reads are named).
    ...
    // ── DOM refs ──────────────────────────────────────────────────────────
    const emailStep   = document.getElementById("email-step") as HTMLFormElement
    const codeStep    = document.getElementById("code-step") as HTMLFormElement
    const emailInput  = document.getElementById("email") as HTMLInputElement
    const otpBoxes    = Array.from(document.querySelectorAll(".otp-box")) as HTMLInputElement[]
    ...
    const sentTo      = document.getElementById("sent-to") as HTMLElement
    const requestStep = document.getElementById("request-step") as HTMLFormElement
    const reqName     = document.getElementById("reqname") as HTMLInputElement
    const reqDept     = document.getElementById("reqdept") as HTMLSelectElement
    const reqEmail    = document.getElementById("req-email") as HTMLElement
    const requestSubmit = document.getElementById("request-submit") as HTMLButtonElement
    const requestBack = document.getElementById("request-back") as HTMLElement
    const msg         = document.getElementById("msg") as HTMLElement
    const emailLoadingDots = document.getElementById("email-loading-dots") as HTMLElement
    ...
    // ── Helpers ───────────────────────────────────────────────────────────
    const showError  = (text: string) => { msg.className = "msg error";  msg.textContent = text }
    const showOk     = (text: string) => { msg.className = "msg ok";     msg.textContent = text }
    const showNotice = (text: string) => { msg.className = "msg notice"; msg.textContent = text }
    const clearMsg   = ()     => { msg.className = "msg";        msg.textContent = "" }
    const busy = (btn: HTMLButtonElement, on: boolean, label: string) => {
        btn.disabled = on
    ...
    otpBoxes.forEach((box, i) => {
        ...
        box.addEventListener("paste", (e: ClipboardEvent) => {
            e.preventDefault()
            // `window.clipboardData` is a legacy (IE-era) fallback with no
            // modern DOM typing — the modern path is e.clipboardData.
    ...
    const goToCodeStep = (email: string) => {
        currentEmail = email
    ...
    liffReady.then(async (inited) => {
        ...
        if (!inited) {
            ...
            try {
                ...
                clientError = "getProfile failed: " + (err && (err as Error).message)
            }
        }
        try {
    ...
    async function attemptSilentLineReauth(idToken: string | null): Promise<boolean> {
    ...
    emailStep.addEventListener("submit", async (e: SubmitEvent) => {
        e.preventDefault()
        clearMsg()
    ...
    codeStep.addEventListener("submit", async (e: SubmitEvent) => {
        e.preventDefault()
        clearMsg()
    ...
    requestStep.addEventListener("submit", async (e: SubmitEvent) => {
        e.preventDefault()
        clearMsg()
