...
(function () {
    ...
    function inputTypeFor(col) {
        if (isTimestampType(col.data_type))
    ...
    function renderRequestCard(reqRow) {
        ...
        card.querySelector(".btn-reject").addEventListener("click", async () => {
            if (!confirm("ปฏิเสธคำขอของ " + (reqRow.name || reqRow.email) + "?"))
                return;
            try {
    ...
    async function loadAccessRequests() {
        try {
            ...
            for (const r of requests)
                requestsList.appendChild(renderRequestCard(r));
            ...
            if (e.message !== "unauthorized")
                console.error("loadAccessRequests failed:", e.message);
        }
    }
    ...
    function fieldLineHtml(col, value, editing) {
        ...
        if (col.column_name === "department") {
            ...
            const opts = options.map((d) => '<option value="' + escHtml(d) + '"' + (d === value ? " selected" : "") + '>' +
                escHtml(d === "INTERN" ? "INTERN" : d) + "</option>").join("");
            return '<div class="field-line"><div class="field-label">' + label + '</div>' +
        ...
        if (col.column_name === "position" && !inputValue)
            inputValue = "นายแพทย์";
        return '<div class="field-line"><div class="field-label">' + label + '</div>' +
    ...
    function collectInputValues(card) {
        ...
        card.querySelectorAll("[data-col]").forEach((el) => {
            ...
            if (col && isNumericType(col.data_type))
                v = Number(v);
            if (col && isTimestampType(col.data_type))
    ...
    function renderRowCard(row) {
        ...
        function renderView() {
            ...
            body.querySelector(".btn-delete").addEventListener("click", async () => {
                if (!confirm("ยืนยันการลบ " + fullName(row) + "?"))
                    return;
                try {
    ...
    function renderList() {
        ...
        for (const row of visible)
            rowsEl.appendChild(renderRowCard(row));
    ...
    async function loadRowsAndColumns(table) {
        ...
        try {
            ...
            if (e.message !== "unauthorized")
                showStatus("โหลดข้อมูลไม่สำเร็จ: " + e.message, true);
        }
        finally {
    ...
    function openAddForm() {
        openFormOverlay(null, "เพิ่ม", async (values) => {
            const { row } = await api("/admin/api/tables/" + encodeURIComponent(currentTable) + "/rows", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) });
            if (row) {
                allRows.push(row);
                populateDeptFilter();
                renderList();
    ...
    function openEditForm(row, pkValue, head, onSaved) {
        openFormOverlay(row, "บันทึก", async (values) => {
            const { row: updated } = await api("/admin/api/tables/" + encodeURIComponent(currentTable) + "/rows/" + encodeURIComponent(pkValue), { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) });
            ...
            if (row.department) {
                if (badge)
                    ...
                    head.insertBefore(Object.assign(document.createElement("div"), { className: "dept-badge", textContent: row.department }), head.querySelector(".chevron"));
            }
            else if (badge) {
                badge.remove();
            }
    ...
    (async function main() {
        ...
        try {
            ...
            if (currentTable)
                await loadRowsAndColumns(currentTable);
            await loadAccessRequests();
        }
        catch (e) {
            if (e.message !== "unauthorized")
                showStatus("โหลดไม่สำเร็จ: " + e.message, true);
        }
        finally {
            load.style.display = "none";
        }
