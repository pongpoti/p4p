...
/**
 * A physician opened /upload/ — not a submission, so it has no counterpart
 * in automation/telegram.js and isn't part of the vendored pair above (there
 * is no email-path equivalent of "someone looked at a page"). Fires on every
 * load, no throttling: a physician re-opening or reloading counts every
 * time, by design.
 *
 * Only the email is known at this point — the page hasn't told the server
 * which month it's about to show, and looking up the roster name here would
 * add a DB round-trip to every page load for a message that doesn't need it.
 */
function formatPageOpenMessage(email) {
  return [
    `👀 P4P Upload — Page Opened`,
    ``,
    `👤 Account: ${email || "—"}`,
  ].join("\n");
}
...
module.exports = { sendTelegram, formatResultMessage, formatErrorMessage, formatPageOpenMessage }
