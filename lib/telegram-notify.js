...
async function sendTelegram(text) {
  ...
  if (!token || !chatId) {
    // Not an error state on a fresh deployment — the two variables are the
    // worker's, and Vercel needs its own copies. Logged every time rather
    // than once: these lines are how a silent chat gets diagnosed.
    console.warn("[upload] TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID not set on this deployment — admin alert skipped")
    return false
  }
  try {
