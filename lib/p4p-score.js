...
// Month number (1-12) -> tokens used to pick the right sheet in a
// multi-sheet workbook. Vendored from automation/months.js's
// MONTH_TOKENS_BY_NUM (sheet-name matching only — MONTH_FOLDER_NAMES is
// Drive-specific and has no reason to exist here).
...
async function parseWorkbookRows(buffer, opts = {}) {
  ...
  const ws = sheets[idx]
  return { rows: rowsFromWorksheet(ws), sheetName: ws.name, sheetCount: sheets.length }
}
