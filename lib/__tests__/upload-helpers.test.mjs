...
test("the deadline is the 5th of the following month, 23:59:59 +07:00", () => {
  ...
  assert.equal(
    ...
    new Date("2026-05-05T23:59:59+07:00").toISOString()
  )
  assert.equal(
    P4P.deadlineDate("2569_12").toISOString(),
    new Date("2027-01-05T23:59:59+07:00").toISOString(),
...
test("lateness is decided by when the file was handed over", () => {
  ...
  // 23:55 on the 5th and drained at 00:05 must still count as on time (§9),
...
test("the deadline renders in Bangkok time regardless of the device clock", () => {
  assert.equal(P4P.deadlineDisplay("2569_06"), "5 ก.ค. 23:59")
})

test("the กำหนดส่ง label shows the due date without the time", () => {
  assert.equal(P4P.deadlineDueDisplay("2569_06"), "ภายใน 5 ก.ค.")
})
