 ...
 * `assets/shared.js` (the page's banner) and `main.js` (the authoritative
...
/**
 * main.js's copy of the same rule. Lifted out of the source rather than
...
function loadServerDeadline() {
  const source = readFileSync(resolve(repoRoot, "main.js"), "utf8")
  const start = source.indexOf("function monthDeadlineMs")
  if (start === -1) throw new Error("main.js no longer defines monthDeadlineMs — re-point this test")
  const open = source.indexOf("{", source.indexOf(")", start))
  let depth = 0
  for (let i = open; i < source.length; i++) {
    if (source[i] === "{") depth++
    else if (source[i] === "}" && --depth === 0) {
      return new Function(`${source.slice(start, i + 1)}; return monthDeadlineMs`)()
    }
  }
...
const receipt = require(resolve(repoRoot, "lib/line-receipt-flex.js"))
...
test("main.js and assets/shared.js agree on every month of two years", () => {
...
test("the six-month window matches src/constants.cjs's MONTH_ITERATOR", () => {
  const { MONTH_ITERATOR } = require(resolve(repoRoot, "src/constants.cjs"))
  for (let m = 0; m < 12; m++) {
