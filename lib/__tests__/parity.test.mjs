...
// rides main.js's existing dependency, fetch in automation/).
const telegramCanonical = readFileSync(resolve(repoRoot, "automation/telegram.js"), "utf8")
const telegramVendored = readFileSync(resolve(repoRoot, "lib/telegram-notify.js"), "utf8")

for (const name of ["uploadWarnings", "accountLine", "formatResultMessage", "formatErrorMessage"]) {
  test(`lib/telegram-notify.js's ${name} is identical to automation/telegram.js's`, () => {
    assert.equal(
