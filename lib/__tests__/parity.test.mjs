 ...
 * `automation/claude-analyst.ts` — it carries a vendored copy in
 * `lib/p4p-score.ts` instead. `web/lib/__tests__/parity.test.ts` already
...
/**
 * Both sides of every comparison below are TypeScript now. Strip types
 * before doing anything else — a plain text diff of two independently
 * hand-typed copies would fail on annotation wording alone (different
 * return-type names, `unknown` vs `any`, etc.) even when the actual logic
 * is byte-identical. `ts.transpileModule` in ESNext-out mode removes only
 * type syntax and leaves `export`/`const`/braces exactly where they were,
 * so the existing brace-matching extractor below still works unmodified.
 */
...
const vendored = require(resolve(repoRoot, "lib/p4p-score"))
const canonicalSource = stripTypes(readFileSync(resolve(repoRoot, "automation/claude-analyst.ts"), "utf8"))
const vendoredSource = stripTypes(readFileSync(resolve(repoRoot, "lib/p4p-score.ts"), "utf8"))
...
// lib/p4p-score.ts that exists in automation/ means adding it here too.
const MIRRORED = [
...
for (const name of MIRRORED) {
  test(`lib/p4p-score.ts's ${name} is identical to automation/claude-analyst.ts's`, () => {
    assert.equal(
...
// ── Second vendored pair: the admin's Telegram message ────────────────────
// Same situation, different file. `/upload/score` sends its own §7.6 alert —
// the worker never sees a row the instant tier resolved — and the admin scans
// both messages by shape, so a layout change that lands in one copy only is
// exactly the drift this guard exists to catch. Only the four formatting
// functions are mirrored: the transport differs on purpose (axios here so it
// rides main.ts's existing dependency, fetch in automation/).
const telegramCanonical = stripTypes(readFileSync(resolve(repoRoot, "automation/telegram.ts"), "utf8"))
const telegramVendored = stripTypes(readFileSync(resolve(repoRoot, "lib/telegram-notify.ts"), "utf8"))

for (const name of ["uploadWarnings", "accountLine", "formatResultMessage", "formatErrorMessage"]) {
  test(`lib/telegram-notify.ts's ${name} is identical to automation/telegram.ts's`, () => {
    assert.equal(
      normalise(extractDeclaration(telegramVendored, name)),
      normalise(extractDeclaration(telegramCanonical, name)),
      `${name} has drifted between the two copies — fix BOTH, or the admin's ` +
      `Telegram alert reads differently depending on which tier scored the file`
    )
  })
}
...
test("both copies return the same {score, method} for every fixture", async (t) => {
  let canonical
  try {
    canonical = await import(resolve(repoRoot, "automation/claude-analyst.ts"))
  } catch (err) {
