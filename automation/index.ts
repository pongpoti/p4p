...
import type { Row, GmailAttachment, MessageWithAttachments, RosterMatch, UploadIdentity, NotifyHooks, ProcessOutcome } from "./types.js";
...
function displayMonthKey(key: unknown): string {
...
let _drive: ReturnType<typeof createDriveClient> | null = null;
function getDrive() {
...
function isOfficeLockFile(filename: unknown): boolean {
...
function rowsOfSheet(worksheet: ExcelJS.Worksheet): Row[] {
  const rows: Row[] = [];
  ...
  worksheet.eachRow((row) => {
    ...
    row.eachCell({ includeEmpty: false }, (cell, colNumber) => {
      ...
      if (val === null || val === undefined) {
        ...
        obj[key] = String((val as { text?: unknown }).text ?? "");
      } else {
        obj[key] = val as Row[string];
      }
    });
 ...
 */
async function stripFormulasFromBuffer(inputBuffer: Buffer): Promise<Buffer> {
...
export async function extractFirstSheetBuffer(buffer: Buffer): Promise<Buffer | null> {
  ...
  await source.xlsx.load(strippedBuffer as unknown as ExcelJS.Buffer);
...
export interface ProcessBufferContext {
  ...
  // Required on the email path (sendAlertReply/gmail.sendMessage need it) —
  // every code path below that touches `gmail` is only reached when
  // `identity` is null (isUpload === false), so it is asserted non-null
  // there rather than threaded through as a second required field. The
  // LINE-upload path (drain-uploads.ts) never has a Gmail client at hand and
  // never needs one: notify hooks own delivery on that path instead.
  gmail?: GmailClient;
  source?: "email" | "line-upload";
  identity?: UploadIdentity | null;
 ...
 * @param context.replyTo      Sender email address for auto-reply
 * @param context.messageId    Gmail message ID for thread reply
 * @param context.gmail        Shared Gmail client instance
 ...
 * @param context.monthKey  "2569_06" — set → routing skips
 *   month inference, but the file's own month is still computed and compared.
 * @param context.notify  Replaces the inline Gmail replies
...
export async function processBuffer(buffer: Buffer, {
  ...
  const uploadCtx = isUpload
    ? {
        ...
        monthKey,
        rosterMatch : (identity!.rosterIndex != null ? "exact" : null) as string | null,
        nameInFile  : null as string | null,   // filled in after Claude has read the sheet
        monthInFile : null as string | null,
        attempt     : identity!.attempt ?? null,
        maxAttempts : 3,
      }
    : null;
  const tgError = (extra: Record<string, unknown>) => (uploadCtx ? { ...uploadCtx, ...extra } : null);
  ...
  const notifyFailure = async (errorType: string = "other", { detail = "", detectedDate = "", detectedName = "", statedDate = "" }: { detail?: string; detectedDate?: string; detectedName?: string; statedDate?: string } = {}): Promise<void> => {
    if (isUpload) {
  ...
  const otherReply = (detail: string = "") => notifyFailure("other", { detail });
  ...
  try {
    ...
    await sendTelegram(formatErrorMessage(`Workbook parse failed: ${message}`, filename, tgError({ errorType: "other" }) as never)).catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
    await otherReply(`ไม่สามารถอ่านไฟล์ได้: ${message}`);
    return "replied";
  }
  ...
  if (rows.length === 0) {
    ...
    console.error(`│        ❌  ${msg}`);
    await sendTelegram(formatErrorMessage(msg, filename, tgError({ errorType: "other" }) as never)).catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
  ...
  if (monthKey) {
    ...
    if ((fileMonth && fileMonth !== selMonth) || (fileYear && fileYear !== selYear)) {
      ...
      await sendTelegram(formatErrorMessage(detail, filename, tgError({ errorType: "month_mismatch" }) as never))
        .catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
  ...
  let analysis: { name: string; date: string; score: number };
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    const isZero = /score is 0/i.test(message);
    console.error(`│        ❌  Analysis failed: ${message}`);
    await sendTelegram(formatErrorMessage(message, filename, tgError({ errorType: isZero ? "zero_score" : "other" }) as never)).catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
    if (isZero) {
      await notifyFailure("zero_score", { detail: "ไม่พบคะแนนรวมในไฟล์" });
    } else {
      await otherReply(message);
    }
    return "replied";
  }
  ...
  let match: RosterMatch | null = null;
  if (isUpload) {
    ...
    try {
      ...
      console.error(`│        ❌  Supabase match error: ${message}`);
      await sendTelegram(formatErrorMessage(`Supabase match error: ${message}`, filename, tgError({ errorType: "other" }) as never))
        .catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
      await otherReply(message);
      return "replied";
    }
  ...
  try {
    ...
    const message = dbErr instanceof Error ? dbErr.message : String(dbErr);
    console.error(`│        ❌  Supabase match error: ${message}`);
    const isTableMissing = /does not exist|undefined_table|42P01|schema cache/i.test(message);
    if (isTableMissing) {
    ...
    if (!match) {
      ...
      await sendTelegram(
        formatErrorMessage(
          ...
          filename,
          tgError({ errorType: "not_in_roster" }) as never
        )
      ).catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
    } else {
      ...
      try {
        ...
        await sendTelegram(
          formatErrorMessage(`Drive upload failed: ${message}`, filename ?? null)
        ).catch((e) => console.warn(`│        ⚠️  Telegram notify failed: ${e.message}`));
        await otherReply();
  ...
  if (scoreSaved && match) {
    try {
      ...
      console.warn(`│        ⚠️  Submission log skipped (non-fatal): ${logErr instanceof Error ? logErr.message : logErr}`);
    }
    ...
    if (!isUpload) try {
      ...
      console.warn(`│        ⚠️  sender_physician_match update skipped (non-fatal): ${matchLogErr instanceof Error ? matchLogErr.message : matchLogErr}`);
    }
  }
  ...
  try {
    await sendTelegram(formatResultMessage({
      ...
      saved      : scoreSaved,
    }, filename ?? null, uploadCtx));
    console.log(`│        ✅  Telegram message sent.`);
  } catch (tgErr) {
    console.error(`│        ❌  Telegram error: ${tgErr instanceof Error ? tgErr.message : tgErr}`);
  }
 ...
 */
async function processAttachment(att: GmailAttachment, messageId: string, context: Partial<ProcessBufferContext>, gmail: GmailClient, workbookCount: number = 1): Promise<ProcessOutcome | false> {
  ...
  return processBuffer(buffer, { ...context, filename: att.filename, gmail, workbookCount } as ProcessBufferContext);
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.warn(`⚠️   Could not resolve label: ${err instanceof Error ? err.message : err}`);
  }
  ...
  const messages = [
    ...inboxMessages.map((m) => ({ ...m, _sourceLabel: "INBOX" as const })),
    ...spamMessages.map((m) => ({ ...m, _sourceLabel: "SPAM" as const })),
  ];
  ...
  type FetchedMessage = MessageWithAttachments & { id: string; _sourceLabel: "INBOX" | "SPAM" };
  const fetchResults = await Promise.allSettled(
    messages.map(async (m): Promise<FetchedMessage> => ({ ...m, ...(await gmail.getMessageWithAttachments(m.id!)), id: m.id! }))
  );
  ...
  for (let i = 0; i < fetchedMessages.length; i++) {
        ...
        try {
          ...
          for (const tm of [...threadMsgs].reverse()) {
            ...
            if (xlsxInMsg.length > 0) {
              ...
              const relayContext: Partial<ProcessBufferContext> = {
              ...
              if (relayProcessed) {
                ...
                try {
                  ...
                  console.error(`│  ❌  Failed to update message labels: ${err instanceof Error ? err.message : err}`);
                }
              }
              break;
            }
          ...
          console.warn(`│  ⚠️   Relay thread search failed: ${err instanceof Error ? err.message : err}`);
        }
        if (!relayProcessed) {
    ...
    if (xlsxAtts.length === 0) {
      ...
      let threadXlsx: { messageId: string; atts: GmailAttachment[] } | null = null;
      if (msg.threadId) {
        try {
          const threadMsgs = await gmail.getThreadMessages(msg.threadId);
          ...
          console.warn(`│  ⚠️   Thread search failed: ${err instanceof Error ? err.message : err}`);
        }
      }
