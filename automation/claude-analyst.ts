...
import type { Row, ScoreResult, MonthYear, Period, StatedPeriods, AnalysisResult } from "./types.js";

/** Minimal shape needed from an ExcelJS worksheet — just its tab name. */
 ...
 */
export function resolveBeYearFromRows(rows: Row[]): number | null {
...
function fourDigitBeYear(text: unknown): number | null {
 ...
 */
export function statedPeriods(filename: unknown, subject: unknown, body: unknown): StatedPeriods {
...
export function monthFromCellText(text: unknown): number | null {
  const s = String(text ?? "");
  for (const [token, mo] of MONTH_TOKEN_MAP) {
    if (/^[A-Za-z]+$/.test(token)) {
...
export function monthYearFromText(text: unknown): MonthYear {
  const s = String(text ?? "");
 ...
 */
export function monthYearFromRows(rows: Row[]): MonthYear {
  for (const row of rows.slice(0, 15)) {
    for (const val of Object.values(row)) {
      if (val === null || val === undefined) continue;
...
export function resolveBeMonthFromRows(rows: Row[]): number | null {
 ...
 */
function isNonName(token: string): boolean {
 ...
 */
function extractNameFromText(text: string | null | undefined): string | null {
  if (!text) return null;
  ...
  let m2;
  let singleTokenFallback: string | null = null; // best firstname when no full pair is found
 ...
 */
export function resolvePhysicianName(filename: unknown, subject: unknown, body: unknown): string | null {
 ...
 *
 * @returns ordered, de-duplicated candidates (may be empty)
 */
export function resolvePhysicianNameCandidates(filename: unknown, subject: unknown, body: unknown): string[] {
 ...
 *
 * @param rows       Sheet rows ({ col_1, col_2, ... })
 * @param sheetName  The chosen worksheet's tab name
 */
export function resolvePhysicianNameFromSheet(rows: Row[] = [], sheetName: string = ""): string[] {
  const candidates: string[] = [];
  const add = (n: string | null) => { if (n && !candidates.includes(n)) candidates.push(n); };
...
const includesLabel = (text: string, label: string) => stripSpace(text).includes(stripSpace(label));
 ...
 * @param val
 * @param skipYearFilter - when true, accept year-like integers as scores
 */
function numsFromText(val: unknown, skipYearFilter: boolean = false): number[] {
 ...
 * @param rows
 * @returns every total stated in prose, in sheet order
 */
 ...
 * @param rows
 * @param skipYearFilter - when true, accept numbers in the year range
 *   (used for confirmed grand-total label rows where the score itself may be year-like)
 */
function collectCandidates(rows: Row[], skipYearFilter: boolean = false): number[] {
  const results: number[] = [];
  for (const row of rows) {
    for (const val of Object.values(row)) {
      const n = toNum(val);
 ...
 *
 * @param rows
 * @returns the column key (e.g. "col_5"), or null when the sheet
 ...
 * @param rows
 */
export function extractScoreFromRows(rows: Row[]): ScoreResult {
...
/** @returns reconstructed grand total, or null if the sheet has no usable header/day columns */
 ...
 */
export function resolveScore(rows: Row[]): ScoreResult {
...
/** Shape of the object analyseJson() is called with. */
export interface AnalyseJsonInput {
  _email_subject?: string;
  _email_body?: string;
  _source_file?: string;
  _selected_sheet?: string;
  _date_key: unknown;
  rows?: Row[];
}

/**
 * @param jsonData  { _email_subject, _email_body, _source_file, _selected_sheet, _date_key, rows[] }
 * @param filename
 */
export async function analyseJson(jsonData: AnalyseJsonInput, filename: string = "data.json"): Promise<AnalysisResult> {
...
${rowsJson}`;
  ...
  let parsed: { name?: unknown; score?: unknown };
  try {
