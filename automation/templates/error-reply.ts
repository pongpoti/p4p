 ...
 *
 * @param data.safeFilename    Attachment filename(s) (may be empty)
 * @param data.errorType  one of the known error types (unknown values fall back to "other")
 * @param data.detectedDate  Date string extracted from file, shown when errorType="wrong_date"
 * @param data.detectedName  Name extracted from file, shown when errorType="physician_not_found"
...
export interface ErrorReplyData {
  safeFilename?: string;
  errorType?: string;
  detectedDate?: string;
  detectedName?: string;
...
export function buildHtmlErrorReply({ safeFilename = "", errorType = "other", detectedDate = "", detectedName = "", statedDate = "" }: ErrorReplyData): string {
  ...
  const CONTENT: Record<string, ErrorContent> = {
    ...
    no_period: {
      ...
      bannerBody   : "ระบบไม่พบเดือนที่ท่านต้องการส่งในหัวข้ออีเมล เนื้อหาอีเมล หรือชื่อไฟล์<br>เพื่อป้องกันการบันทึกคะแนนผิดเดือน ระบบจะไม่เดาเดือนให้",
      instruction  : "กรุณาระบุเดือนในหัวข้ออีเมล เช่น \"ส่ง P4P เดือน ก.ค. 2569\" หรือตั้งชื่อไฟล์เป็น \"P4P ก.ค. 2569.xlsx\" แล้วส่งใหม่อีกครั้ง",
    },
    ambiguous_period: {
