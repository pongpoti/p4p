...
function sortDeptsThai<T extends { dept: string }>(list: T[]): T[] {
 ...
 */
export function isMonthComplete(deptStatuses: ({ complete: boolean } | null)[]): boolean {
  const present = deptStatuses.filter((s): s is { complete: boolean } => s !== null);
...
async function getSentMonthsByDept(sb: SupabaseClient, monthKeys: string[]): Promise<Map<string, Set<string>>> {
  ...
  for (const { table_name, department } of data as { table_name: string; department: string }[]) {
...
const logSentFailures: LogSentFailure[] = [];

async function logEmailSent(sb: SupabaseClient, tableKey: string, department: string): Promise<void> {
  const upsertOnce = () => sb
    .from("email_sent_log")
    .upsert(
...
export async function main() {
  ...
  for (const monthKey of monthsAsc) {
    ...
    const statusByDept = new Map<string, DeptStatus | null>();
    ...
    if (!isMonthComplete([...statusByDept.values()])) {
      const pending = [...statusByDept.entries()]
        .filter((entry): entry is [string, DeptStatus] => entry[1] !== null && !entry[1].complete)
    ...
    const byEmail = new Map<string, { dept: string; status: DeptStatus }[]>();
    ...
    const sentThisMonth: { dept: string; status: DeptStatus }[] = []; // for every dept actually sent — feeds the oversight email
...
// ── Step summary helper ────────────────────────────────────────────────────
function writeSummary(rows: { month: string; dept: string; emailed: boolean; note: string }[], monthsAsc: string[], todayStr: string): void {
