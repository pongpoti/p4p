...
async function children(api: drive_v3.Drive, folderId: string): Promise<drive_v3.Schema$File[]> {
 ...
 */
async function stripPublic(api: drive_v3.Drive, id: string, label: string): Promise<void> {
  stats.scanned++;
  ...
  try {
    ...
    console.warn(`  ⚠️  cannot read permissions on ${label}: ${message}`);
    stats.failed++;
    return;
  }
  ...
  for (const p of perms) {
    ...
    if (!APPLY) continue;
    try {
      await api.permissions.delete({ fileId: id, permissionId: p.id ?? undefined });
      stats.publicRemoved++;
    } catch (err) {
      // Inherited permissions are removed at the ancestor, not here.
      const message = err instanceof Error ? err.message : String(err);
      if (!/inherited/i.test(message)) {
        console.warn(`     could not remove: ${message}`);
        stats.failed++;
      }
    }
  }
...
async function grantReader(api: drive_v3.Drive, id: string, email: string, label: string): Promise<void> {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.warn(`     grant failed: ${message}`);
    stats.failed++;
  }
}

/** Depth-first strip of public access over the whole subtree. */
async function stripTree(api: drive_v3.Drive, id: string, label: string, depth = 0): Promise<void> {
...
async function main() {
  const roots = [process.env.P4P_FOLDER_ID, ...EXTRA_ROOTS].filter((r): r is string => Boolean(r));
...
main().catch((err: unknown) => {
  console.error("fix-drive-sharing failed:", err instanceof Error ? err.message : String(err));
  process.exit(1);
});
