...
import type { GmailAttachment, MessageWithAttachments } from "../types.js";
...
// ── Helpers ──────────────────────────────────────────────────────────────
const addrOf = (fromHeader: string): string =>
  (fromHeader.match(/<(.+?)>/) ?? [, fromHeader])[1]!.trim().toLowerCase();

const isBot = (fromHeader: string): boolean => BOT_ADDRESSES.has(addrOf(fromHeader));

const isExcel = (att: GmailAttachment): boolean =>
...
function parseSuccessReply(body: string): { rawName: string; workMonth: string } | null {
...
async function main() {
  ...
  const byKey = new Map<string, FoundSubmission>();
  ...
  // Report per month
  const perMonth: Record<string, FoundSubmission[]> = {};
  ...
  if (!DRY_RUN) {
    for (const s of submissions) {
      try { await logSubmission(s); written++; }
        ...
        const message = e instanceof Error ? e.message : String(e);
        failed++; console.error(`❌  ${s.physicianName} ${s.workMonth}: ${message}`);
      }
    }
