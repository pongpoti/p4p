...
import { matchName, saveSenderMatch, type SaveSenderMatchInput }   from "../supabase-client.js";
...
async function listAllIds(gmail: gmail_v1.Gmail, labelId: string): Promise<string[]> {
  const ids: string[] = [];
...
async function getRosterTables(): Promise<string[]> {
...
async function tryMessage(gmailClient: GmailClient, rawGmail: gmail_v1.Gmail, msgId: string, tables: string[]): Promise<TryMessageResult | null> {
...
async function main() {
  ...
  if (!labelId) throw new Error(`Label "${LABEL_NAME}" resolved with no id`);
  ...
  const senderMap = new Map<string, SenderGroup>(); // email → { displayName, email, messageIds[] }
  ...
  for (let i = 0; i < allIds.length; i++) {
    ...
    if (!existing) {
      ...
      existing.messageIds.push(allIds[i]!);
    }
  ...
  const results: SaveSenderMatchInput[] = [];
  ...
  for (const { displayName, email, messageIds } of senderMap.values()) {
    ...
    let result: TryMessageResult | null = null;
    ...
    const senderMatch: SaveSenderMatchInput = result
      ? {
