...
(function () {
    ...
    async function loadData() {
        ...
        try {
            ...
            if (myToken !== loadToken)
                return; // stale — a newer load already started
            if (error)
                throw error;
            const rows = data || [];
            allData = rows.map(r => ({
    ...
    function renderTable() {
        document.getElementById('rowCountDisplay').textContent = String(filtered.length);
